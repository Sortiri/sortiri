...
import { markEvalRunRunningViaIngest } from "@/lib/sortiri/evalApi";
...
export async function POST(req: Request, context: RouteContext) {
  ...
  try {
    const result = await markEvalRunRunningViaIngest(
      auth,
    ...
    const message = err instanceof Error ? err.message : "Failed to mark eval run running";
