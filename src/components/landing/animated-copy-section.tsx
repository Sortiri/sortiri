    ...
    >
      ...
      >
        ...
        {lines.map((line) => {
          ...
          return (
            <p key={line.text} className={`${BASE_FONT_CLASS} text-center ${landing.sectionHeadline}`}>
              {tokens.map((token, index) => (
                <motion.span
                  key={`${token.word}-${index}`}
                  className={`inline${token.accent ? ` ${ACCENT_CLASS} ${ACCENT_FONT_CLASS} ${landing.sectionHeadlineAccent}` : ""}`}
                  variants={wordVariants}
                >
                  {token.word}
